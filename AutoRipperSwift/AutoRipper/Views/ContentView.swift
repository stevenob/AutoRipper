    ...
    var body: some View {
        NavigationSplitView {
            List(selection: $selectedTab) {
                Section {
                    ForEach([AppTab.disc, .queue, .history, .failed, .library], id: \.self) { tab in
        ...
        .onAppear {
            ripVM.onRipComplete = { [weak queueVM] rip in
                queueVM?.addJob(rip)
            }
...
struct DiscPaneView: View {
    ...
    private func titlesAndLogColumn(info: DiscInfo) -> some View {
        VStack(spacing: 0) {
            TitlesTableView(ripVM: ripVM, config: config, info: info)
            Divider()
            logPane
        }
        .frame(minWidth: 480)
